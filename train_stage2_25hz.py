...
def add_fourth_layer_to_decompressor(model, device):
    ...
    decomp.in_proj = nn.Linear(1024, 1024).to(device)
    with torch.no_grad():
        nn.init.eye_(decomp.in_proj.weight)
        nn.init.zeros_(decomp.in_proj.bias)
...
class FocalCodecTrainer:
    ...
    def compute_loss(
        ...
        if self.weight_time > 0:
            min_len = min(reconstructed_audio.shape[-1], audio.shape[-1])
            recon_aligned = reconstructed_audio[..., :min_len]
            orig_aligned = audio[..., :min_len]
...
def main():
    ...
    stage1_ckpt = os.path.join(config.output_dir, 'stage1_50hz', 'best_model.pt')
    if not os.path.exists(stage1_ckpt):
        print(f"ERROR: Stage 1 checkpoint not found: {stage1_ckpt}")
        print("Please run Stage 1 first: python train_stage1_50hz.py")
        return
    ...
    # Training loop
    checkpoint_dir = os.path.join(config.output_dir, 'stage2_25hz')
