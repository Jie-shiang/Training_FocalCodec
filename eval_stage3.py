#!/usr/bin/env python3
"""
dCER Evaluation Script for Stage 3 FocalCodec (12.5Hz)
...
Usage:
    python eval_stage3.py --config config_exp_E.yaml
    python eval_stage3.py --config config_exp_E.yaml --max_samples 500
    python eval_stage3.py --config config_exp_E.yaml --asr whisper
...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate dCER for Stage 3 FocalCodec (12.5Hz)')
...
def main():
    ...
    print("="*60)
    print(f"  dCER Evaluation: Stage 3 ({frame_rate}Hz {bits}-bit)")
    print("="*60)
    ...
    if not os.path.exists(inference_dir):
        ...
        print(f"Run inference first: python infer_stage3.py --config {args.config}")
        return 1
    ...
    # Print summary
    print("\n" + "="*60)
    print(f"  EVALUATION RESULTS - Stage 3 ({args.asr.upper()})")
    print("="*60)
    print(f"  ASR Model: {args.asr}")
    ...
    # Compare with Stage 1 and Stage 2 if available
    print("\n" + "="*60)
    print("  COMPARISON WITH OTHER STAGES")
    print("="*60)

    # inference_dir from config already includes experiment_name
    stage1_dir = os.path.join(config.inference_dir, 'stage1')
    stage2_dir = os.path.join(config.inference_dir, 'stage2')

    stage1_summary = os.path.join(stage1_dir, f'dcer_summary_{args.asr}.json')
    stage2_summary = os.path.join(stage2_dir, f'dcer_summary_{args.asr}.json')

    comparison_data = [
        {
            'stage': 'Stage 3 (12.5Hz)',
            'bitrate': 150,
            'dcer': summary['dCER_mean']
        }
    ]

    if os.path.exists(stage1_summary):
        with open(stage1_summary) as f:
            stage1_data = json.load(f)
            comparison_data.insert(0, {
                'stage': 'Stage 1 (50Hz)',
                'bitrate': 550,
                'dcer': stage1_data['dCER_mean']
            })

    if os.path.exists(stage2_summary):
        with open(stage2_summary) as f:
            stage2_data = json.load(f)
            comparison_data.insert(-1 if len(comparison_data) > 1 else 0, {
                'stage': 'Stage 2 (25Hz)',
                'bitrate': 275,
                'dcer': stage2_data['dCER_mean']
            })

    for data in comparison_data:
        print(f"  {data['stage']:20s} | {data['bitrate']:3.0f} bps | dCER: {data['dcer']:.4f} ({data['dcer']*100:.2f}%)")
